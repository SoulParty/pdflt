...
public class PdfServiceImpl implements PdfService {
	...
	/*
	   ...
	   */
	@Override
	public PresignData prepareToSign(PdfReader reader, OutputStream pdfPresignOutputStream, SignatureProperties signatureProperties,
		...
		data.setCertificateChain(new X509Certificate[] { certificate });
...
//		}
	...
	@Override
	public PresignData prepareToLtv(byte[] bytesToSign, OutputStream pdfPresignOutputStream, SignatureProperties signatureProperties,
			X509Certificate
					certificate)
			throws Exception {
	...
	public PdfPKCS7 prepareSignature(PresignData data) throws GeneralSecurityException {
		...
		byte bytesToSign[] = sgn
				.getAuthenticatedAttributeBytes(data.getSecondDigest(), data.getSigningDate(), data.getOcspBytes(), null, CRYPTO_STANDARD);
	...
	public void sign(InputStream pdfPresignInputStream, OutputStream pdfOutputStream, PresignData data, byte[] signatureBytes) throws Exception {
		...
		byte[] encodedSig = sgn.getEncodedPKCS7(data.getSecondDigest(), data.getSigningDate(), null, data.getOcspBytes(), null, CRYPTO_STANDARD);
		...
		if (currentEstimate + 2 < encodedSig.length) {
			...
			Exception exception = new Exception(
					"Not enough space to write signature. We have reserved " + currentEstimate + "b and actual signature is "
...
//    }
		...
		if (signatureProperties.getPosition() != null) {
			...
			if (isVisible) {
				...
				textBuilder.append(CertificateUtils.getSubjectCN(certificateChain[0]));
				textBuilder.append("\n").append(String.valueOf(data.getSigningDate().getTime()));
				textBuilder.append("\n").append(signatureProperties.getReason());
				textBuilder.append("\n").append(signatureProperties.getLocation());
				textBuilder.append("\n").append(signatureProperties.getContact());
				...
				Font signatureFont = FontFactory
						.getFont("signature_font", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, Font.UNDEFINED, Font.UNDEFINED, BaseColor.BLACK,
								BaseFont.CACHED);
...
//        throw new DocumentException("Document is signed and version is too low " + reader.getPdfVersion() + " while minimal PDF-LT supported
// version is "
//            + MIN_PDF_LT_VERSION);
			} else {
	...
	private void renterLayer2Description(PdfSignatureAppearance sap) throws DocumentException {
		...
		ct.setSimpleColumn(new Phrase(sap.getLayer2Text(), font), dataRect.getLeft(), dataRect.getBottom(), dataRect.getRight(), dataRect.getTop(),
				size,
				Element.ALIGN_LEFT);
		ct.go();
	}
