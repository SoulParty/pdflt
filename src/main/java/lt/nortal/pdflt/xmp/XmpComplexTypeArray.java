    ...
    if ((tag = ent.getClass().getAnnotation(XmlType.class)) != null) {
      for (String prop : tag.propOrder()) {
        ...
        try {
          ...
          throw new RuntimeException("Unable to get field value for " + ent.getClass() + ", no such field.", e);
        } catch (IllegalAccessException e) {
          throw new RuntimeException("Unable to get field value for " + ent.getClass() + ", field not accessible.", e);
        } catch (SecurityException e) {
          throw new RuntimeException("Unable to get field value for " + ent.getClass() + ", nu such field.", e);
        } catch (InvocationTargetException e) {
          throw new RuntimeException("Unable to get field value for " + ent.getClass() + ", method fail.", e);
        } catch (NoSuchFieldException e) {
          throw new RuntimeException("Unable to get field value for " + ent.getClass() + ", no such value.", e);
        }
        ...
        if (val != null) {
          ...
          if (val instanceof String) {
            ...
            try {
              xc = DatatypeFactory.newInstance().newXMLGregorianCalendar((GregorianCalendar) val);
            } catch (DatatypeConfigurationException e) {
              throw new RuntimeException("Date conversion error.", e);
            }
            valText = xc.toXMLFormat();
          } else {
